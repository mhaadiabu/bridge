...
async function getStudentAssignmentRows(ctx: QueryCtx, studentProfileId: Id<"profiles">) {
  const recipients = await ctx.db
    ...
    .withIndex("by_student_due", (q) => q.eq("studentProfileId", studentProfileId))
    .collect();
...
export const getDetail = query({
  ...
  handler: async (ctx, args) => {
    ...
    const submission = await ctx.db
      .query("submissions")
      .withIndex("by_assignment_student", (q) =>
        q.eq("assignmentId", recipient.assignmentId).eq("studentProfileId", viewer._id),
      )
      .first();
    ...
    const nudges = await ctx.db
      ...
      .withIndex("by_student_assignment", (q) =>
        q.eq("studentProfileId", viewer._id).eq("assignmentId", assignment._id),
      )
      .collect();
    ...
    return {
      ...
      nudgeHistory: nudges
        ...
        .map((nudge) => ({
          ...
          deliveryStatus: nudge.deliveryStatus,
          sentAt: nudge.sentAt,
          openedAt: nudge.openedAt,
          adaptationReason: nudge.adaptationReason,
        })),
    };
  },
});
...
export const recordSubmission = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existing = await ctx.db
      .query("submissions")
      .withIndex("by_assignment_student", (q) =>
        q.eq("assignmentId", assignment._id).eq("studentProfileId", viewer._id),
      )
      .first();
