...
export const createExperiment = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (args.assignmentMethod !== "manual") {
      const membersSorted = [...members].sort((a, b) => (a.studentProfileId < b.studentProfileId ? -1 : 1));
      let indexPointer = 0;
      for (let i = 0; i < args.groups.length; i += 1) {
        const group = args.groups[i];
        const groupId = groupIds[i];
        const memberCount =
          i === args.groups.length - 1
    ...
    const rows = await Promise.all(
      experiments.map(async (experiment) => {
        ...
        const assignments = await ctx.db
          ...
          .withIndex("by_experiment_student", (q) => q.eq("experimentId", experiment._id))
          .collect();
        return {
          ...experiment,
          groups,
      ...
      groups.map(async (group) => {
        ...
        for (const studentId of studentIds) {
          submissions.push(
            ...(await ctx.db
              .query("submissions")
              .withIndex("by_student", (q) => q.eq("studentProfileId", studentId))
              .collect()),
          );
        }

        const metrics = studentIds.length > 0 ? computeSubmissionMetrics(submissions) : createEmptySubmissionMetrics();
        const groupNudges = nudgeEvents.filter((event) => event.groupId === group._id);
        const sent = groupNudges.filter((event) => event.deliveryStatus === "sent" || event.deliveryStatus === "opened");
