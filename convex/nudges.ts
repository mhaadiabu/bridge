...
async function resolveExperimentContext(ctx: MutationCtx, studentProfileId: Id<"profiles">) {
  ...
  const group = await ctx.db.get(latest.groupId);
...
export const listForViewer = query({
  ...
  handler: async (ctx, args) => {
    ...
    const all = await ctx.db
      .query("nudgeEvents")
      .withIndex("by_student_scheduled", (q) => q.eq("studentProfileId", viewer._id))
      .collect();

    const includeScheduled = args.includeScheduled ?? true;
    const filtered = includeScheduled ? all : all.filter((event) => event.deliveryStatus !== "scheduled");
...
export const dispatchDueNudges = mutation({
  ...
  handler: async (ctx) => {
    ...
    const dueScheduled = due.filter((event) => event.deliveryStatus === "scheduled" && event.scheduledFor <= now);
...
export const generateForViewer = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const recipients = await ctx.db
      ...
      .withIndex("by_student_due", (q) => q.eq("studentProfileId", viewer._id))
      .collect();

    const candidateRecipients = recipients.filter((recipient) => recipient.status !== "submitted" && recipient.assignmentDueAt >= now);
    const submissions = await ctx.db
      .query("submissions")
      .withIndex("by_student", (q) => q.eq("studentProfileId", viewer._id))
      .collect();
    ...
    const signals = deriveBehaviorSignals(submissions, existingEvents);
    ...
    for (const recipient of candidateRecipients) {
      ...
      const strategy = experimentContext?.strategyId
        ...
        : activeStrategies.find((s) => s.type === plan.type) ?? activeStrategies[0];

      if (!strategy) continue;

      const templates = await ctx.db
        .query("nudgeTemplates")
        .withIndex("by_strategy_active", (q) => q.eq("strategyId", strategy._id).eq("isActive", true))
        .collect();
      ...
      for (const offsetHours of plan.scheduleOffsetsHours) {
        ...
        const message = template ? interpolateTemplateBody(template.body, assignment.title, dueLabel) : fallbackCopy.message;
